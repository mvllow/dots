...
local function map(mode, lhs, rhs, desc, bufnr)
	vim.keymap.set(mode, lhs, rhs, { silent = true, desc = desc or '', buffer = bufnr })
end
...
local packer_path = vim.fn.stdpath('data') .. '/site/pack/packer/start/hotpot.nvim'
	...
	use({
		...
		config = function()
			local function on_attach(client, bufnr)
				...
				map('n', '<leader>a', vim.lsp.buf.code_action, 'code action', bufnr)
				map('n', '<leader>r', vim.lsp.buf.rename, 'rename symbol', bufnr)
				map('n', 'K', vim.lsp.buf.hover, 'symbol hover', bufnr)
